...
class PyMulSim:
  ...
  def __compute_similarities(self, data_multi_graph1:[], data_multi_graph2:[], graph1_node_labels:[]=None, graph2_node_labels:[]=None, model_type:str='GNN', channels:int=64, epochs:int=0, verbose:bool=False):
    ...
    model = None
    if model_type == 'GNN':
      model = GNN(in_channels=channels, hidden_channels=channels, out_channels=channels, num_layers=num_layers)
    elif model_type == 'GIN':
      model = GIN(in_channels=channels, hidden_channels=channels, out_channels=channels, num_layers=num_layers)
    else:
      if verbose: print("Model's Id ('my_model_id') not recognized.")
      exit()
    ...
    for layer_i in range(0, num_layers):
      ...
      for epoch in range(epochs):
          ...
          loss.backward()
          optimizer.step()
          if verbose: print("Epoch %d (loss=%d)" % (epoch, float(loss)))

      with torch.no_grad():
        model.eval()

        if verbose: print("--- Node embedding...")
        # Using GNNAlignmentModel for processing aligned features of each layer
        embeddings_layer_i_graph1 = model(layer_i_graph1.x, layer_i_graph1.edge_index, layer_i_graph1.edge_index_inter)
        embeddings_layer_i_graph2 = model(layer_i_graph2.x, layer_i_graph2.edge_index, layer_i_graph2.edge_index_inter)

        # Normalization
        if verbose: print("--- Normalization...")
        ...
        for i in range(len(embeddings_layer_i_graph1)):
            for j in range(len(embeddings_layer_i_graph2)):
                ...
                if similarity > 0:
                  sims.append([node_name_graph1, node_name_graph2, similarity])
                  if verbose: print(f"{node_name_graph1} (Graph1) - {node_name_graph2} (Graph2) : {similarity}")

    return np.asarray(sims)

  ####
        
  def compute_similarities_from_file(self,source_path:str, target_path:str, layer_ids:[], interlayer_ids:[], model_type:str='GNN', channels:int=64, epochs:int=0, verbose:bool=False):
    data_multi_graph1, data_multi_graph2, graph1_node_labels, graph2_node_labels = read_data_from_file(source_path, target_path, layer_ids, interlayer_ids)
    return self.__compute_similarities(data_multi_graph1, data_multi_graph2, graph1_node_labels, graph2_node_labels, model_type, channels, epochs, verbose)

  def compute_similarities_from_nx(self,graph1_layers:nx.Graph, graph2_layers:nx.Graph, model_type:str='GNN', channels:int=64, epochs:int=0, verbose:bool=False):
    data_multi_graph1, data_multi_graph2, graph1_node_labels, graph2_node_labels = read_data_from_nx(graph1_layers, graph2_layers)
    return self.__compute_similarities(data_multi_graph1, data_multi_graph2, graph1_node_labels, graph2_node_labels, model_type, channels, epochs, verbose)
